...
// Note: The the API read response uses a different structure than the create request.
...
type readInstanceResponse struct {
	...
	PrivateIP      string   // first private IP from addresses
	PublicIP       string   // first public IP from addresses
}
...
// extractFirstIP normalises the heterogeneous shapes the API uses for the
// "private" / "public" entries inside `addresses`. The single-instance GET
// returns a list of {"addr": "<ip>", ...} objects; the list endpoint returns
// a bare IP string. Returns "" if neither shape applies.
func extractFirstIP(v interface{}) string {
	if s, ok := v.(string); ok {
		return s
	}
	if list, ok := v.([]interface{}); ok && len(list) > 0 {
		if obj, ok := list[0].(map[string]interface{}); ok {
			if addr, ok := obj["addr"].(string); ok {
				return addr
			}
		}
	}
...
func parseInstanceData(data json.RawMessage) (*readInstanceResponse, error) {
	...
	// addresses on the getOne route is:
	//   {"public":  [{"version":4,"addr":"<ip>","type":"floating",...}, ...],
	//    "private": [{"version":4,"addr":"<ip>","type":"fixed",   ...}, ...]}
	// (whereas the list / e2e route returns flat strings — handled below).
	// Extract the first address from each list, falling back to the flat
	// string shape if that's what the API returned.
	if addrs, ok := raw["addresses"].(map[string]interface{}); ok {
		resp.PrivateIP = extractFirstIP(addrs["private"])
		resp.PublicIP = extractFirstIP(addrs["public"])
	}
...
func (r *instanceResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// Try single object first (the API cloud wrapper), then array fallback.
...
// model. The the API read response does NOT return many create-time fields
...
func mapReadResponseToState(ctx context.Context, instance *readInstanceResponse, state *instanceResourceModel) diag.Diagnostics {
	...
	state.PrivateIP = types.StringValue(instance.PrivateIP)
	state.PublicIP = types.StringValue(instance.PublicIP)
