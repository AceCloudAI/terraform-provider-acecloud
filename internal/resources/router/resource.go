...
// Uses map[string]interface{} to exactly match the platform serialization behavior.
func buildCreateRequest(plan *routerResourceModel) map[string]interface{} {
	...
	// External gateway info — always send the key,
...
// NOTE: the API router PUT rejects "description" — only name, admin_state_up,
