...
func (r *keyPairResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// The the API keypair create DTO maps the JSON input field `key` to the
	...
	// private_key is returned only on create, only when the caller did NOT
	// supply their own public_key. Capture it for SSH-key workflows. If the
	// user supplied public_key, set to empty string so the Computed field
	// has a known value.
	if v, ok := result["privateKey"].(string); ok && v != "" {
		plan.PrivateKey = types.StringValue(v)
	} else {
...
func (r *keyPairResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	// private_key is intentionally not refreshed from the API — it was
	// returned only on create, and keeping the existing state value avoids
	// spurious drift on every refresh.
