...
import (
	...
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/auth"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/client"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/datasources/flavors"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/datasources/images"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/datasources/routers"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/datasources/security_groups"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/datasources/vpcs"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/api_key"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/auto_scaling_deployment"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/auto_scaling_template"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/floating_ip"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/floating_ip_association"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/instance"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/key_pair"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/lb_health_monitor"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/lb_listener"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/lb_pool"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/lb_pool_member"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/load_balancer"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/router"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/router_interface"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/security_group"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/snapshot"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/subnet"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/volume"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/volume_attachment"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/volume_backup"
	"github.com/AceCloudAI/terraform-provider-acecloud/internal/resources/vpc"
...
func (p *AceCloudProvider) Schema(_ context.Context, _ provider.SchemaRequest, resp *provider.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "Terraform provider for Ace Cloud infrastructure. Manages compute instances, volumes, snapshots, backups, VPCs, subnets, routers, security groups, floating IPs, key pairs, and load balancers via the Ace Cloud API. Authenticate by setting `api_key_id`, `api_key_secret`, and `api_key_service_name` in the provider block, or via the matching `ACECLOUD_API_KEY_*` environment variables. Create a key from the AceCloud console.",
		Attributes: map[string]schema.Attribute{
			...
			"region": schema.StringAttribute{
				Description: "Cloud region the provider operates against (for example `ap-south-noi-1`, `ap-south-mum-2`, or `us-east-at-2`). Required. Set via the provider block or the `ACECLOUD_REGION` environment variable. Region codes follow the pattern `<geo>-<city>-<index>`; the set of regions enabled on your account is shown in the AceCloud console. There is no auto-selection — every configuration explicitly names its target region.",
				Required:    true,
			},
...
func (p *AceCloudProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	// api_url chain: explicit HCL/env var → local credentials file → built-in
